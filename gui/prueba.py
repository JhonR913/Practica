...
"}")
    ...
    def consultar_camara(self):
        """Devuelve un diccionario con los datos de la primera cámara guardada, o None."""
        try:
            conn = connect_to_database()
            cursor = conn.cursor(dictionary=True)
            cursor.execute("SELECT ip, puerto, usuario, password FROM camaras LIMIT 1")
    ...
    def cargar_camara_guardada(self):
        """Si existe cámara en BDD, la muestra en camara1Frame con doble‑clic habilitado."""
        ...
        ip   = datos["ip"]
        port = str(datos["puerto"])
        user = datos["usuario"]
        pwd  = datos["password"]

        rtsp_url = f"rtsp://{user}:{pwd}@{ip}:{port}/Streaming/Channels/101"
        self.last_rtsp_url = rtsp_url
        print(f"Cargando cámara guardada: {rtsp_url}")
        ...
        if hasattr(self, 'rtsp_thread') and self.rtsp_thread and self.rtsp_thread.isRunning():
            print("Reutilizando hilo RTSP, reconectando señal")
            try:
                self.rtsp_thread.change_pixmap.disconnect()
            except Exception:
                pass
